...
| PE-G11 | **EniBundle + apply --force** — `EniBundle` manifest kind for full ENI + dependency chain config in one YAML (auto-expands to vnet→eni→vnet_mapping→route_policy→acl_policy in tier order with auto-wired FK references). Create-vs-update detection on `dashctl apply`: new objects CREATE, existing objects BLOCKED with warning + instructions (`--force` to overwrite, `diff` to preview, `replace` for individual). Same `--force` flag on `dash-sim-client apply`. 12 EniBundle unit tests + live e2e. | ✅ |
