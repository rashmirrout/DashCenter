...
- What objects exist for this ENI, VNET, tenant, or workload?
- Why did this packet match, redirect, or drop?
- What path did this flow take through the DPU?
- Is the configured state fully realized on the device?
- Is an ENI ready to migrate to another DPU, and what flows are still active?
- Which DASH devices in the fleet are degraded right now?

## What it provides

### Central service

DashCenter is designed as a central service that manages one or many DASH-compliant devices from a single control point. Operators connect to DashCenter instead of logging into each device independently, which makes it suitable for both single-node operations and fleet-scale environments.

### `dashctl` CLI

The companion CLI, `dashctl`, provides a kubectl-style operator experience for querying and debugging objects, flows, and fleet state from a terminal. Commands are object-aware, scriptable, and work consistently across one device or many devices.
...
|---|---|
| Get object | Inspect ENIs, VNETs, ACLs, routes, mappings, services, and runtime state in one normalized view. |
| Explain match | Show why a packet or flow matched a given DASH policy/path, including winning rule and failure point. |
| Trace flow | Show the expected and observed path of traffic through the DPU and related DASH objects. |
| Reconcile state | Compare intended/configured state against APP_DB, STATE_DB, and runtime realization. |
| ENI mobility | Build ENI migration bundles, show flow ownership, check readiness, and monitor drain state. |
| Fleet visibility | View health, findings, and degradation across many DASH-compliant devices from one place. |
| Evidence collection | Generate support bundles with snapshots, findings, logs, and flow/path context for RCA. |

## Architecture at a glance

DashCenter is built around a central analysis service with pluggable collectors and adapters. The service reads DASH-facing APIs and runtime data sources, normalizes them into a canonical object graph, then serves diagnostics workflows to CLI, API, and UI consumers.
...
Returns the ENI, its VNET, attached ACLs, route dependencies, meter/service bindings, realization status, and current health findings.
...
Shows inferred ENI, matched ACL stage/rule, selected route or mapping, meter decision, and final forwarding or drop action.
...
Shows the packet journey across classification, ENI selection, policy, routing, mapping, service path, and egress realization.
...
Compares intended/configured state to APP_DB, STATE_DB, and runtime realization and highlights drift, missing dependencies, or partial programming.
...
Shows whether the target DPU is ready to host the ENI, what rules and dependencies must move, and whether active flows or ownership state make cutover risky.
...
DashCenter is currently in design and architecture phase. The initial target is a gRPC-first service with a `dashctl` CLI, followed by fleet APIs, streaming health, historical snapshots, and a web console.
