...
| 30 | **EniBundle + apply --force** — `EniBundle` manifest kind for full ENI + dependency chain in one YAML file (auto-expands to vnet→eni→vnet_mapping→route_policy→acl_policy in tier order with auto-wired FKs). `dashctl apply --force` for create-vs-update detection: new objects created, existing objects blocked with warning + instructions (use --force to overwrite, diff to preview, replace for individual). Same `--force` flag on `dash-sim-client apply`. | ✅ Done | 2026-06-15 | 2026-06-15 | **Delivered**: `manifest/bundle.go` (EniBundle expander + auto-wiring), `apply.go` (create-vs-update via Get, --force flag, BLOCKED/CREATE/MODIFY output, summary line, WARNING instructions), `dash-sim-client/apply.go` (same --force + existence check). 12 EniBundle unit tests (100% on ExpandBundles/strField/copyMap, 90.2% on expandEniBundle). Live e2e: 5 CREATE → 5 BLOCKED → 5 MODIFY with --force. All 45 packages green. |
