...
| 30 | **Bundles + apply --force** — 4 bundle manifest kinds (`EniBundle`, `AclBundle`, `RouteBundle`, `HaBundle`) for full DASH construct + dependency chain in one YAML file (auto-expands to individual specs in tier order with auto-wired FKs). `dashctl apply --force` for create-vs-update detection: new objects created, existing objects blocked with warning + instructions (use --force to overwrite, diff to preview, replace for individual). Same `--force` flag on `dash-sim-client apply`. | ✅ Done | 2026-06-15 | 2026-06-15 | **Delivered**: `manifest/bundle.go` (4 bundle expanders: EniBundle→5 specs, AclBundle→3 specs, RouteBundle→4 specs, HaBundle→1 spec), `apply.go` (create-vs-update, --force, BLOCKED/CREATE/MODIFY), `dash-sim-client/apply.go` (same). 20 bundle unit tests + live e2e. All 45 packages green. |
