...
| Phase 2 · PE · Diagnostics & gNMI | ⏳ PE-1 diagnostics ✅, PE-3a/b/c counters ✅; **PE-4 sim parity planned** (rows #23–#25: flow table + transforms + HA) |
...
| # | What | Status | Started | Done | Notes |
|---|---|---|---|---|---|
| 26 | **RI Phase 1** — dash-sim + dash-sim-client Apply-side FK validation for all 25 southbound `dashapi.v1` FK relationships. Bottom-layer defense: the sim is the lowest layer and the ONLY validation point for direct `dash-sim-client apply`. Config `--strict-refs` flag (default true). New `dash-sim-client validate <file>` subcommand for dry-run FK checks. | ⬜ Not started | — | — | **Scope**: extend `model.Store.Apply()` with FK checks using a declarative FK rules table. Return gRPC `FailedPrecondition` with clear error in `Ack.error` naming the missing ref. ~20 UTs + update existing integration tests to create objects in correct tier order. **Estimated effort**: ~2-3 days. **Design doc**: [referential-integrity-validation.md](dashd-features/referential-integrity-validation.md) §8 Phase 1. |
| 27 | **RI Phase 2a** — dashd Put-side FK validation for the 7 missing northbound FKs. Declarative FK registry + inline conditional checks. Config `validation.referential_integrity: strict\|warn\|off`. Reject with 400 / `FailedPrecondition` on missing ref. | ⬜ Not started | — | — | **Scope**: extend `namespace.Validator` + new `internal/referential/validator.go`. Covers ENI→qos, ENI→meter_policy×2, RoutePolicy→service_tunnel, VnetMapping→tunnel, HaSet→DPU IDs. Batch-aware. ~15 UTs. **Depends on**: #26 (sim validated first). **Estimated effort**: ~2 days. |
| 28 | **RI Phase 2b** — dashd Delete-side orphan protection. Reject `DELETE` with 409 Conflict when dependents still reference the target object. Force-delete via `?force=true` with WARN audit entry. | ⬜ Not started | — | — | **Scope**: `CheckDelete(ctx, ns, kind, name)` scans for dependents across all referencing kinds. Covers vnet, eni, service_tunnel, qos, meter_policy, ha_set. ~10 UTs. **Depends on**: #27. **Estimated effort**: ~1 day. |
| 29 | **RI Phase 3** — Operational tooling: `dashctl validate` + `dash-sim-client validate` subcommands + `GET /v1/diagnostics/validate-refs` REST endpoint + SPA card under `/diagnostics`. Scans entire store for existing dangling references. | ⬜ Not started | — | — | **Scope**: store-wide orphan scan + report at both sim and dashd layers. Critical for migrating existing deployments to strict mode. ~5 UTs + live e2e. **Depends on**: #26 + #27. **Estimated effort**: ~1 day. |
