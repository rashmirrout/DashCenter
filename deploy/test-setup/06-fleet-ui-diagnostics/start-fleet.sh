#!/usr/bin/env bash
# 06-fleet-ui-diagnostics/start-fleet.sh — bring up the full DashCenter stack.
#
...
while [[ $(date +%s) -lt $deadline ]]; do
    for port in 37443 37453 37463; do
...
if [[ $SKIP_CONTEXT -eq 0 ]]; then
    ...
    if [[ -n "$DASHCTL" ]]; then
        "$DASHCTL" config set-context fleet-ui-diagnostics \
            ...
            --admin-endpoint http://127.0.0.1:37443 >/dev/null
        "$DASHCTL" config use-context fleet-ui-diagnostics >/dev/null
        echo "==> dashctl context 'fleet-ui-diagnostics' active (using $DASHCTL)"
    else
        echo "   (dashctl not found in fleet dir, ../04-ha-fleet, or PATH — skipped context setup)"
        echo "   Build it once: (cd ../../../src/impl-go/dashctl && go build -o ../../../deploy/test-setup/06-fleet-ui-diagnostics/dashctl ./cmd/dashctl)"
    fi
fi

echo ""
echo "Per-node REST/admin endpoints (host):"
echo "  dashd-1: http://127.0.0.1:38443  (admin :37443)"
echo "  dashd-2: http://127.0.0.1:38453  (admin :37453)"
echo "  dashd-3: http://127.0.0.1:38463  (admin :37463)"
[[ $WITH_CONSOLE -eq 1 ]] && echo "  dashw  : http://localhost:3001   (web console)"
echo ""
