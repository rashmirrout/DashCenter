#!/usr/bin/env pwsh
# 05-full-console/start-fleet.ps1 — bring up the full DashCenter stack.
#
...
if (-not $SkipContext) {
    ...
    if ($dashctl) {
        ...
        & $dashctl config use-context full-console | Out-Null
        Write-Host "==> dashctl context 'full-console' active (using $dashctl)" -ForegroundColor Green
    } else {
        Write-Host "   (dashctl not found in fleet dir, ../04-ha-fleet, or PATH — skipped context setup)" -ForegroundColor DarkGray
        Write-Host "   Build it once: pushd ../../../src/impl-go/dashctl; go build -o ../../../deploy/test-setup/05-full-console/dashctl.exe ./cmd/dashctl; popd" -ForegroundColor DarkGray
    }
}
...
if ($WithConsole) {
    Write-Host "  dashw  : http://localhost:3000   (web console)"
}
Write-Host ""
