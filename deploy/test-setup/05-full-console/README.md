...
| **Provision** | `provision.{ps1,sh}` | applies `manifest/*.yaml` via `dashctl apply -R -f` (or `bootstrap.py` fallback) |
...
# 3. Load the rich 157-object superset
...
```bash
./start-fleet.sh --with-console && ./provision.sh && open http://localhost:3000
```

```powershell
pwsh ./start-fleet.ps1 -WithConsole; pwsh ./provision.ps1; Start-Process http://localhost:3000
```

## Pre-loaded Data

`bootstrap.py` PUTs a **rich superset of 157 objects across 3 namespaces**
...
| **RoutePolicy**   | **19** | Per-tenant defaults + **3-way ECMP** (weighted 50/30/20) + gaming blackhole (drop precedence over scrub) |
| **AclPolicy**     | **20** | Tenant inbound/outbound + multi-ns + advanced: numeric protos (`6`,`1`,`17`,`58`), port ranges (`7777-7800`,`1024-65535`), `src_ports`, `allow_and_continue` chains |
...
See [manual-handson.md](manual-handson.md) for a 12-lab guided tour.
...
- [`manifest/bootstrap.py`](manifest/bootstrap.py) — Python loader for the 157-object superset (used as provision.sh fallback)
